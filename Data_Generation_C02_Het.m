...
for j = 1:MC_sim     
    ...
    err = reshape(err,T,N)';
    Rdelta = reshape(delta_hat,T,N,3);
    Rdelta = permute(Rdelta, [2, 1, 3]);
    dm = zeros(N,T);
    ...
    for i = 1:N
        g = opt_group_assign(i);
        for t = 1:T
            dm(i,t) = x1(t,i)*thetas(1,g) + x2(t,i)*thetas(2,g) + x3(t,i)*thetas(3,g) + x4(t,i)*thetas(4,g) + x5(t,i)*thetas(5,g) + x6(t,i)*thetas(6,g) + x7(t,i)*thetas(7,g) + x8(t,i)*thetas(8,g) + Rdelta(i,t,g) + err(i,t);
        end
    end
    Y0 = reshape(dm',N*T,1);
    clear Y
...
fileID1 = fopen('inputMCXG3_het_CO2_changed.txt', 'w');
fileID2 = fopen('inputMCYG3_het_CO2_changed.txt', 'w');
